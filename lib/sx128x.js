...
SX128x.prototype.readBuffer = async function (offset=0x00,payloadLen=252) {
    ...
    let sendCommand = {
        ...
        receiveBuffer : Buffer.alloc(payloadLen+3)
    };
...
SX128x.prototype.receive = async function (continuous = true, keep_listening=true) {
    ...
    if (continuous){
        this._irqMask = IRQ_RADIO_ALL;
        this.dioMask.dio1Mask = IRQ_RX_DONE+IRQ_RX_TX_TIMEOUT;
        await this.setDioIrqParams(this._irqMask, this._dioMask);
        await this.setRx();
        await this.setStandby('STDBY_RC');
        ...
        if (this._bufStatus[0] > 252){
            this._bufStatus[0] = 252;

        }
        this._packetLen = this._bufStatus[0];
        this._packetPointer = this._bufStatus[1];
        let rxEnd = this._bufStatus[1] + this._bufStatus[0];
