...
SX128x.prototype.writeBuffer = async function (data, offset=0x00) {
    ...
    /*    return new Promise ((resolve, reject) => this._spi.transfer([writeMessage], (err, messages)=>{
        ...
        */
        console.log('write buffer send message');
        console.log(sendMsg);
...
SX128x.prototype.readBuffer = async function (offset=0x00,payloadLen=252) {
    ...
    let _msgArray = [0x1B, offset, 0xFF];
    for (var i=0; i <= payloadLen;i++){
        _msgArray.push(NOP);
    }
    console.log('readBuff sendArray');
    console.log(_msgArray);

    let readBuffer =  await this._sendCommand(Buffer(_msgArray));
    this.trace('readBuffer ' + readBuffer[0].slice(3));
    return readBuffer[0].slice(3);

    /*let sendCommand = {
        sendBuffer    : Buffer(_msgArray),
        byteLength    : _msgArray.length,
        receiveBuffer : Buffer.alloc(_msgArray.length)
    };
