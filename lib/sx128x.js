...
SX128x.prototype.send = async function (data) {
    ...
    */
    ...
    if (msg.length <= 252 ){
        ...
        await this.setDioIrqParams(this._irqMask, this._dioMask);
        await this.setTx();
        if (await this._dio1Gpio.readSync()){
            this._trace('dio1 High');
            return this.irqParse();
        }
        else{
            this._trace('dio1 watch')
            await this._dio1Gpio.watch((err, value) ={
                if(err){
                    throw err;
                }
                this._trace('DIO IRQ triggered!');
                return this.irqParse()
            });
        }
    else{
