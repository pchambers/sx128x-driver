...
SX128x.prototype.readBuffer = async function (offset=0x00,payloadLen=252) {
    ...
    if(payloadLen>252){
        console.error('Error: readBuffer payload length larger than 252-byte maximum`');
    }
    let cmd = Buffer([this.READ_BUFFER, offset, NOP]);
    let sendCommand = {
        sendBuffer    : cmd,
        byteLength    : cmd.length,
        receiveBuffer : Buffer.alloc(payloadLen)
    };
