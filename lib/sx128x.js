...
SX128x.prototype.readBuffer = async function (offset=0x00,payloadLen=252) {
    ...
    let _msgArry = [];
    _msgArry.push(0x1B); //For whatever reason this.READ_BUFFER caused the outcome to be undefined.
    //console.log('rd buff: ' +this.READ_BUFFER.toString('hex'));
    _msgArry.push(offset);
    for (i=0;i<payloadLen+1;i++){
        //_msgArry.push(NOP);
    }
    let sendCommand = {
        sendBuffer    : Buffer(_msgArry),
        byteLength    : _msgArry.length,                 //_msgArry.length,
        receiveBuffer : Buffer.alloc(payloadLen+3)
    };
    //confirmed the buffer sent was the same as the buffer that was in teh callback.
    //console.log('Input sendBuffer: '+ sendCommand.sendBuffer.toString('hex'));

    return new Promise ((resolve, reject) => this._spi.transfer([sendCommand], (err, messages) => {
        if(err){
            reject(err);
        }
        //console.log('output sendBuffer ' + messages[0].sendBuffer.toString('hex'));
        resolve(messages[0].receiveBuffer);
    }));
...
SX128x.prototype.setDioIrqParams = async function (irqMask, {dio1Mask, dio2Mask,dio3Mask}) {
    ...
    */
    ...
    _msgArry[0] = this.SET_DIO_IRQ_PARAMS;
    irqMask.forEach((item, i) => {
        _msgArry.push(item);
    });
...
SX128x.prototype.getRxBufferStatus = async function () {
    ...
    return await this._sendCommand(Buffer([this.GET_RX_BUFFER_STATUS,NOP]));
    //console.log('getRxBufferStatus res[0]' + response[0]);
    //console.log('getRxBufferStatus res[1]' + response[1]);
    //console.log('getRxBufferStatus res[2]' + response[2]);
    //console.log('getRxBufferStatus res[3]' + response[3]);
...
SX128x.prototype.send = async function (data) {
    ...
    */
    ...
    if (msg.length <= 252 ){
        ...
        console.log(await this.readBuffer(0x00,8));
        //readBuffer not matching write buffer, only returning 255bye array with 0xFF 00 00..
        //await setDioIrqParams(this._irqMask, this._dioMask); //not implemented on hardware
        await this.setTx();
    }
};
...
SX128x.prototype.receive = async function (continuous = true, keep_listening=true) {
    ...
    if (continuous){
        ...
        if (this._packetLen > 0){
            ...
            let packet = await this.readBuffer(0x00,9);                                     //try to hard-code offset and packet length
