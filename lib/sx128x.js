...
SX128x.prototype._writeRegister = async function (addr, data) {
    let sendBuffer= Buffer([this.WRITE_REGISTER, addr, data]);
    //this._trace('Writing to: '+ addr1 +' ' + addr2);
    await this._sendCommand(sendBuffer);

};

SX128x.prototype._readRegister = async function (addr) {
    //this._trace('Reading: ' + addr1 + ' ' +addr2);

    let msg = new Buffer([this.READ_REGISTER, addr]);
    let readMessage = {
        sendBuffer: msg,
...
SX128x.prototype.setHighSensitivity = async function () {
    ...
    let lnaRegime = await this._readRegister(this.REG_LNA_REGIME);
...
SX128x.prototype.setRangingParams = async function (range_addr, master, slave) {
    ...
    if (slave){
        ...
        this._writeRegister(0x919, range_addr[0]);
        this._writeRegister(0x918, range_addr[1]);
        this._writeRegister(0x917, range_addr[2]);
        this._writeRegister(0x916, range_addr[3]);
    }
    else if (master){
        this._rangingRole = this.RANGING_MASTER;
        //Master Ranging Address
        this._writeRegister(0x915, range_addr[0]);
        this._writeRegister(0x914, range_addr[1]);
        this._writeRegister(0x913, range_addr[2]);
        this._writeRegister(0x912, range_addr[3]);
    }
    else {
    ...
    this._writeRegister(0x931,0x30);
    //Ranging Calibration - SF7/BW1600=13528=0x34D8 per Section 3.3 of SemTech AN1200.29
    this._writeRegister(0x92D, 0x04);
    this._writeRegister(0x92C, 0x28);
    //Set Ranging Role
...
SX128x.prototype.readRange = async function (raw) {
    ...
    let _temp, _conf, _val, _valLSB;
     _temp = await this._readRegister(0x97F) | (1<<1);
    await this._writeRegister(0x97F, _temp);
    // Set the ranging type for filtered or raw
    _conf = await this._readRegister(0x9024);
    if(raw){
    ...
    _val = await 0 | (this._readRegister(0x9061)<<16);
    _val |= await (this._readRegister(0x9062)<<8);
    _val |= await (this._readRegister(0x9062));
...
SX128x.prototype._getFreqInt = async function () {
    ...
    let msb, mid, lsb;
    if(this._packetType == this.PACKET_TYPE_LORA){
        msb = await this._readRegister(0x906);
        mid = await this._readRegister(0x907);
        lsb = await this._readRegister(0x908);
    }
    if(this._packetType == this.PACKET_TYPE_FLRC){
        msb = await this._readRegister(0x9A3);
        mid = await this._readRegister(0x9A4);
        lsb = await this._readRegister(0x9A5);
    }
    console.log(msb);
    console.log(mid);
    console.log(lsb);
