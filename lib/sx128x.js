...
SX128x.prototype.readBuffer = async function (offset=0x00,payloadLen=252) {
    ...
    let _msgArry = [];
    _msgArry.push(this.READ_BUFFER);
    ...
    console.log('_msgArry'+_msgArry.length + ' payloadLen: ' +payloadLen );
    console.log('_msgArry          ' + _msgArry[0]);
    console.log('Input readBuffer: '+ sendCommand.sendBuffer.toString('hex'));
    return new Promise ((resolve, reject) => this._spi.transfer([sendCommand], function(err, messages) {
        if(err){
            reject(err);
        }
        console.log('output readBuffer ' + messages[0].sendBuffer.toString('hex'));
        resolve(messages[0].receiveBuffer);
    }));
