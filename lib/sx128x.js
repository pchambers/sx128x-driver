...
SX128x.prototype.setModulationParams = async function ({spreadingFactor, bandwidth, codingRate}) {
    ...
    if(this._packetType == this.PACKET_TYPE_LORA){
        if(spreadingFactor == this.LORA_SF_5 || this.LORA_SF_6){
            await this._writeRegister(0x925,0x1E);
        }
        else if (spreadingFactor == this.LORA_SF_7 || this.LORA_SF_8){
            await this._writeRegister(0x925,0x37);
        }
        else if (spreadingFactor == this.LORA_SF_9 || this.LORA_SF_10||this.LORA_SF_11||this.LORA_SF_12){
            await this._writeRegister(0x925,0x32);
        }
        else{
            console.error('Invalid Spreading Factor ' + spreadingFactor);
            console.log('this.lora_sf_7 ' + SX128x.LORA_SF_7);
...
SX128x.prototype.send = async function (data) {
    ...
    */
    ...
    if (msg.length <= 252 ){
        ...
        //this is where the writeBuffer code used to be.

        if(this._packetType == this.PACKET_TYPE_LORA){
            let adr1 = this.REG_LR_PAYLOADLENGTH & 0xFF;
            let adr2 = this.REG_LR_PAYLOADLENGTH >> 8;
            await this._writeRegister(adr1, adr2, msg.length);
        }
        if (this._packetType == this.PACKET_TYPE_FLRC){
            this.setPacketParams(this._pktParams);
        }
