...
SX128x.prototype.send = async function (data) {
    ...
    */
    ...
    console.log('data len: ' + msg.length);
    if (msg.length <= 252 ){
        let pktParam = this._pktParams;
        pktParam.pktParam3 = msg.length;
        console.log('In Send: setPacketParams');
        await this.setPacketParams(pktParam);
        console.log('In Send: writeBuffer');
        await this.writeBuffer(msg);
        console.log('In Send: setTx');
        await this.setTx();
        console.log('In Send: Finished Send.');
    }
};
