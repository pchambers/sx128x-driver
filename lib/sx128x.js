...
SX128x.prototype._sendCommand = async function (command) {
    ...
    let sendCommand = {
        ...
        receiveBuffer: new Buffer.alloc(55)
    }
    ...
    return new Promise ((resolve, reject) => this._spi.transfer([sendCommand], function(err, messages) {
        ...
        if(messages[0].hasOwnProperty('receiveBuffer')){
            //console.log('receiveBufferLength: ' + messages[0].receiveBuffer.length);
            //console.log('receiveBuffer: ' + messages[0].receiveBuffer.readUInt8(1));
            resolve(messages[0]);
        }
        else{
...
SX128x.prototype.getIRqStatus = async function (clear) {
    ...
    let _stat;
    _stat = await this._sendCommand(Buffer([GET_IRQ_STATUS, 0x00, 0x00, 0x00]));
    if (clear){
...
SX128x.prototype.getRxBufferStatus = async function () {
    return await this._sendCommand(Buffer([GET_RX_BUFFER_STATUS, 0x00,0x00,0x00]));
};
...
SX128x.prototype.receive = async function (continuous = true, keep_listening=true) {
    ...
    if (continuous){
        ...
        //console.log('BufStatus: '+ this._bufStatus);
        this._packetLen = this._bufStatus[2];
        this.packetPointer = this._bufStatus[3];
        console.log('PacketLen: '+this._packetLen);
