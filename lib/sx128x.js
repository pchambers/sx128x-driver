...
function SX128x (options) {
    //from 128x.py
    //Add Buffer for _BigBuffer = bytearray(257)
    //add Buffer for _status = bytearray(1)
    ...
    this._modParams = options.modParams || {modParam1: LORA_SF_7, modParam2:LORA_BW_400, modParam3:LORA_CR_4_5};
    this._pktParams = options.pktParams || {pktParam1: 0x08, pktParam2:0x00, pktParam3:0x0F, pktParam4:0x00, pktParam5:0x00, pktParam6:0x00, pktParam7:0x00};
    this._txParams = options.txParams || {power: 0x1F, rampTime:0xE0};
    this._irqMask = options.irqMask || {irqMask: [0x40,0x23]};
    this._dioMask = options.dioMask || {dio1Mask:[0x00,0x01], dio2Mask:[0x00,0x02], dio3Mask:[0x40,0x20]};
    ...
    this._dio1Pin = (options.hasOwnProperty('dio1Pin')) ? options.dio1Pin : 18;
    this._packetType = options.packetType || PACKET_TYPE_LORA; //default
    this._regulatorMode = options.regulatorMode || 0x01;
    this._setRanging = options.ranging || False;
    this._ranging = options.ranging || False;
    this._status = options.status || 0;
    this._rfFreq = options.rfFreq || FREQ_24GHZ;
    this._autoFS = options.autoFS || False;
    ...
    //this._dio1Pin = (options.hasOwnProperty('dio1Pin')) ? options.dio1Pin : 18;
    /*
    this._frequency = options.frequency || 2484e6;
...
SX128x.prototype._openSpi = async function(spiBus, spiDevice, SPI_OPTIONS) {
...
SX128x.prototype._trace = function(messgae) {
    if (this._debug == true) {
...
SX128x.prototype.open = async function() {
    ...
    //add _inveretIqReg check?

    //await each setter function
...
SX128x.prototype.methodName = function () {
...
SX128x.prototype._sendCommand = async function (command) {
    let _size = command.length;
    return new Promise ((resolve, reject) => this._spi.transfer([command], function(err, messages) {
        if (err){
            reject(err);
        }
        this._BUFFER = messages[0].receiveBuffer.readUInt8();
        console.log(this._convertStatus(this._status));
        resolve();
    }));
};

SX128x.prototype._writeRegister = async function (addr1, addr2, data) {
    let sendBuffer= Buffer([WRITE_REGISTER, addr1, addr2, data]);
    let writeMessage = {
        sendBuffer: sendBuffer,
        byteLength: sendBuffer.length
    };
    if (this._debug){
        console.log('Writing to: '+ addr1 +' ' + addr2);
    };
    this._sendCommand(writeMessage);
};

SX128x.prototype._readRegister = async function (addr1, addr2) {
    if (this._debug){
        console.log('Reading: ' + addr1 + ' ' +addr2);
    };
    let readMessage = {
        sendBuffer: new Buffer([READ_REGISTER, addr1, addr2]),
...
SX128x.prototype.setRegulatorMode = async function (mode) {
    if (this._debug) {
        console.log('Setting Regulator Mode');
    }
    let sendBuffer = Buffer([SET_REGULATOR_MODE, mode]);
    await this._sendCommand(sendBuffer);
};

SX128x.prototype.setStandby = async function (state) {
    if(this._debug){
        console.log('Setting Standby');
    }
    if(state== 'STDBY_RC'){
        await this._sendCommand(Buffer([SET_STANDBY, 0x00]));
    }
    else if (state == 'STDBY_XOSC'){
        await this._sendCommand(Buffer([SET_STANDBY, 0x01]));
    }
};
...
SX128x.prototype.setPacketType = async function (type) {
    ...
    if (this._debug){
        console.log('Setting Packet Type');
    }
    await this._sendCommand(Buffer([SET_PACKET_TYPE, this._packetType]));
    this.packetType = type;
};

SX128x.prototype.setCadParams = async function (symbol) {
    //default to 0x80
    if (this._debug){
        console.log('Setting CAD Parameters');
    }
    await this._sendCommand(Buffer([SET_CAD_PARAMS,symbol]));
};

SX128x.prototype.setCad = async function () {
    if (this._debug){
        console.log('Setting CAD Search');
    }
    await this._sendCommand(Buffer([SET_CAD]));
...
SX128x.prototype.setRFFreq = async function (freq = [0xB8, 0x9D, 0x88]) {
    ...
    //freq/(52000000/(2**18))
    if(this._debug){
        console.log('Setting RF Frequency');
    }
    await this._sendCommand(Buffer([SET_RF_FREQUENCY]+freq));
};

SX128x.prototype.setBufferBaseAddr = async function (txBaseAddr = 0x00, rxBaseAddr = 0x00) {
    //defaults tx: 0x00, rx: 0x00;
    if (this._debug){
        console.log('Setting Buffer Base Address');
    }
    this._txBaseAddress = txBaseAddr;
    this._rxBaseAddress = rxBaseAddr;
    await this._sendCommand(Buffer([SET_BUFFER_BASE_ADDRESS, txBaseAddr, rxBaseAddr]));
};
...
SX128x.prototype.setModulationParams = async function ({modParam1, modParam2, modParam3}) {
    ...
    if(this._debug){
        console.log('Setting Modulation Parameters');
    }
    await this._sendCommand(Buffer([SET_MODULATION_PARAMS, modParam1, modParam2, modParam3]));
    if(this.packetType == PACKET_TYPE_LORA){
        if(modParam1 == LORA_SF_5 || LORA_SF_6){
            ...
            console.log('Invalid Spreading Factor');
        }
    }
};
...
SX128x.prototype.setTxParam = async function ({power, rampTime}) {
    //defaults=  power:0x1F, rampTime:0xE0
    //power=13 dBm (0x1F), rampTime=20us (0xE0). See Table 11-47
    await this._sendCommand(Buffer([SET_TX_PARAMS, power, rampTime]));
};
...
SX128x.prototype.writeBuffer = async function (data) {
    ...
    _offset = this._txBaseAddress;
    _len = data.length;
    if (0 < _len && len <= 252){
        return new Promise ((resolve, reject) =>this._spi.transfer([Buffer([WRITE_BUFFER, _offset, data])], function(err, messges){
            if(err){
                reject(err);
            }
            resolve(messages[0].receiveBuffer.readUInt8(1));
        ...
        console.log('Error Writing buffer. Length out of Range');
        console.error('Error Writing buffer. Length out of Range');
    }
};

SX128x.prototype.readBuffer = async function (offset) {
    return new Promise ((resolve, reject) =>this._spi.transfer([Buffer([READ_BUFFER, offset])], function(err, messges){
        if(err){
            reject(err);
        }
        //not sure if this is right? supposed to just return message
        this._BUFFER = messages[0].receiveBuffer.readUInt8(1);
        resolve(messages[0].receiveBuffer.readUInt8(1));
        //return messages[0].receiveBuffer.readUInt8(1);
    }));
};

SX128x.prototype.dumpBuffer = async function () {
    let bufferOut = Buffer([READ_BUFFER, 0x00]);
    return new Promise ((resolve, reject) => this._spi.transfer([Buffer([READ_BUFFER,0x00])], function(err, messages){
        if(err){
            reject(err);
        }
        resolve(messages[0].receiveBuffer.readUInt8(1));
...
SX128x.prototype.setDioIrqParams = async function (irqMask, {dio1Mask, dio2Mask,dio3Mask}) {
    ...
    */
    ...
    if(this._debug){
        console.log('Setting Dio IRQ Parameters');
    }
    await this._sendCommand(Buffer([SET_DIO_IRQ_PARAMS]+irqMask+dio1Mask+dio2Mask+dio3Mask));
};

SX128x.prototype.clearIrqStatus = async function (val) {
    //val default = [0xFF, 0xFF]
    if(this._debug){
        console.log('Clearing IRQ Status');
    }
    await this._sendCommand(Buffer([CLR_IRQ_STATUS]+val));
};

SX128x.prototype.getIRqStatus = async function (clear) {
    if(this._debug){
        console.log('Getting IRQ Status');
    }
    let _stat;
    stat = await this._sendCommand(Buffer([GET_IRQ_STATUS, 0x00, 0x00, 0x00]));
    if (clear){
        await this._sendCommand(Buffer([CLR_IRQ_STATUS, 0xFF, 0xFF])); //Clear IRQ status
    }
    return _stat;
};

SX128x.prototype.setTx = async function (pBase, pBaseCount) {
    ...
    await this.clearIrqStatus([0xFF,0xFF])
    await this._sendCommand(Buffer([SET_TX, pBase, pBaseCount[0], pBaseCount[1]]));
};
...
SX128x.prototype.setRx = async function (pBase, pBaseCount) {
    ...
    */
    ...
    if (this._debug){
        console.log('Setting Rx');
    }
    await this.clearIrqStatus([0xFF,0xFF]);
    await this._sendCommand(Buffer([SET_RX, pBase, pBaseCount[0], pBaseCount[1]]))
};

SX128x.prototype.autoFs = async function (value) {
    await this._sendCommand(Buffer([SET_AUTO_FS, value]));
    this._autoFS = value;
};
...
SX128x.prototype.setRangingParams = async function (range_addr, master, slave) {
    ...
    if (slave){
        ...
        console.log('Select Master or Slave Only');
        return False
    }
    ...
    //Set Ranging Role
    await this._sendCommand(Buffer([SET_RANGING_ROLE, this._rangingRole]));
    if (slave){
...
SX128x.prototype.getPacketStatus = async function () {
    // see Table 11-63
    this._packetStatus = [];
    let p_stat = await  this._sendCommand(Buffer([GET_PACKET_STATUS,0x00,0x00,0x00,0x00,0x00,0x00]));
    this.rssiSync = (-1*(p_stat[2])/2);
    this.snr = (p_stat[3]/4);
};

SX128x.prototype.getRxBufferStatus = async function () {
    await this._sendCommand(Buffer([GET_RX_BUFFER_STATUS, 0x00,0x00,0x00]));
    return self._BUFFER;
};
...
SX128x.prototype.send = async function (data) {
    ...
    */
    if (data.length <= 252 ){
        this._pktParams.pktParam3 = data.length;
        await this.setPacketParams(this._pktParams);
        await this.writeBuffer(data);
        await this.setTx([0x02], [0x00,0x00]);
    }
};
...
SX128x.prototype.receive = async function (continuous = True, keep_listening=True) {
    ...
    if (continuous){
        ...
        this.packetPointer = this.bufStatus[3];
        if (this._packetLen > 0){
            if (this._debug){
                console.log('Offset: ' + this._packetPointer + ' Length: ' +this._packetLen);
            }
            let packet = await this.readBuffer(this._packetPointer)
            if(!keep_listening){
                self._listen = False;
            }
...
SX128x.prototype.Rssi = async function () {
    this._rssi = await this._sendCommand(Buffer([GET_RSSI_INST, 0x00,0x00]));
    return this._rssi;
};

SX128x.prototype.status = async function () {
    let a = await this._sendCommand(Buffer([GET_STATUS]))[0]
    if (a){
