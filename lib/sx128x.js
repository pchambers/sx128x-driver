...
SX128x.prototype.getRxBufferStatus = async function () {
    ...
    let response = await this._sendCommand(Buffer([this.GET_RX_BUFFER_STATUS,NOP]));
    ...
    return this._bufStatus = response;
};
...
SX128x.prototype.send = async function (data) {
    ...
    */
    ...
    if (msg.length <= 252 ){
        ...
        await this.writeBuffer(msg);  //console.log(_writeBuff.sendBuffer.slice(2)); //
        console.log(await this.readBuffer(0x80,msg.length));
