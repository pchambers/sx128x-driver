...
SX128x.prototype.send = async function (data) {
    ...
    */
    ...
    if (msg.length <= 252 ){
        ...
        await this.setTxParam(this._txParams);
        //await this.setPacketParams(this._pktParams);
        await this.writeBuffer(msg,0x20);  //console.log(_writeBuff.sendBuffer.slice(2)); //
        console.log(await this.readBuffer(0x20,7));
...
SX128x.prototype.receive = async function (continuous = true, keep_listening=true) {
    ...
    if (continuous){
        ...
        if (this._packetLen > 0){
            ...
            let packet = await this.readBuffer(0x00,17);//this._packetPointer,this._packetLen); //try to hard-code offset and packet length
