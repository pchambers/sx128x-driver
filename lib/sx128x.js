...
SX128x.prototype.printModemSettings = async function () {
    ...
    (this._pktParams.pktParam4 == this.PACKET_FIXED_LENGTH) ? (this._trace('Packet Length Fixed'): (this._pktParams.pktParam4 == this.PACKET_VARIABLE_LENGTH) ? this._trace('Packet Length Variable') : this._trace('Unknown Packet Length Type ' + this._pktParams.pktParam4));
...
SX128x.prototype._getFreqInt = async function () {
    ...
    let msb, mid, lsb;
    if(this._packetType == this.PACKET_TYPE_LORA){
        msb = this._readRegister(0x9, 0x06);
        mid = this._readRegister(0x9, 0x07);
        lsb = this._readRegister(0x9, 0x08);
    }
    if(this._packetType == this.PACKET_TYPE_FLRC){
        msb = this._readRegister(0x9, 0xA3);
        mid = this._readRegister(0x9, 0xA4);
        lsb = this._readRegister(0x9, 0xA5);
    }
