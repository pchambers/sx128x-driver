...
SX128x.prototype.setRegulatorMode = async function (mode) {
    ...
    await this._sendCommand(Buffer([SET_REGULATOR_MODE, mode]));
};
...
SX128x.prototype.writeBuffer = async function (data) {
    ...
    if(Buffer.isBuffer(data)){
        sendBuffer = Buffer.concat([
            Buffer([WRITE_BUFFER, this._txBaseAddress]),
            data
        ]);
    } else{
        sendBuffer = Buffer([WRITE_BUFFER,this._txBaseAddress, data]);
    }
    let writeMessage = {
        sendBuffer : sendBuffer,
        byteLength : sendBuffer.length
    ...
    if (0 < sendBuffer.length && sendBuffer.length <= 252){
        return new Promise ((resolve, reject) =>this._spi.transfer(writeMessage, function(err, messges){
            if(err){
                reject(err);
            }
            resolve(messages[0].receiveBuffer.readUInt8(1));
