...
SX128x.prototype.getPacketStatus = async function () {
    ...
    if(this._packetType == PACKET_TYPE_LORA || PACKET_TYPE_RANGING){
        this.rssiSync = (-1*(this._pktStatus[2])/2);
        this.snr = (this._pktStatus[3]/4);
        return {rssiSync: this.rssiSync, snr: this.snr, status: this._pktStatus[0].toString('hex')};
    }
    else{
        this.rssiSync = (-1*(p_stat[3])/2);
        return {rfu: this._pktStatus[2],rssiSync: this.rssiSync, errors: this._pktStatus[4],status: this._pktStatus[5], sync: this._pktStatus[6], status: this._pktStatus[0].toString('hex')};
    }
};
...
SX128x.prototype.Rssi = async function () {
    this._rssi = await this._sendCommand(Buffer([this.GET_RSSI_INST, 0x00,0x00]));
    return this._rssi;
};
