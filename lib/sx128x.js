...
SX128x.prototype.readBuffer = async function (payloadLen=252,offset=this._rxBaseAddress,) {
    if(!this._spi){
    ...
    return new Promise ((resolve, reject) =>this._spi.transfer([sendCommand], function(err, messages) {
        ...
        resolve(messages[0].receiveBuffer.splice(2));
    }));
...
SX128x.prototype.send = async function (data) {
    ...
    */
    ...
    if (msg.length <= 252 ){
        ...
        console.log(await this.readBuffer(msg.length));
